    ...
    <div className="min-h-screen">
          ...
          {viewingInvoice && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><Label className="text-sm text-gray-600">Дата</Label><p className="text-lg font-semibold">{formatDateUkrainian(viewingInvoice.date)}</p></div>
              </div>
              
              <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                <h3 className="font-semibold text-green-900 mb-2">Контрагент</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div><Label className="text-sm text-gray-600">ЄДРПОУ</Label><p className="font-medium">{viewingInvoice.counterparty_edrpou}</p></div>
                  <div><Label className="text-sm text-gray-600">Назва</Label><p className="font-medium">{viewingInvoice.counterparty_name}</p></div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-4 flex justify-end">
                  <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                    <p className="text-lg font-bold text-green-900">Всього: {viewingInvoice.total_amount.toFixed(2)} грн</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <Dialog open={showActDialog} onOpenChange={closeActDialog}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <DialogFooter className="flex gap-2 justify-end">
            <Button variant="outline" onClick={() => { setEmailType('act'); setShowEmailDialog(true); }}>
              <Mail className="w-4 h-4 mr-2" /> Email
            </Button>
            <Button variant="outline" onClick={downloadActPDF}>
              <Download className="w-4 h-4 mr-2" /> Завантажити
            </Button>
            <Button onClick={previewActPDF}>
              <Eye className="w-4 h-4 mr-2" /> Переглянути PDF
            </Button>
            <Button variant="destructive" onClick={deleteAct}>
              <Trash2 className="w-4 h-4 mr-2" /> Видалити
            </Button>
          </DialogFooter>
        </DialogContent>
