...
const FullDashboard = () => {
  ...
  // Act filters
  const [actsSortBy, setActsSortBy] = useState('date_desc');
  const [actsViewMode, setActsViewMode] = useState('grid2');
  ...
  const saveEditedInvoice = async (formData = editInvoiceForm) => {
    if (!viewingInvoice) return;
  ...
  return (
    <div className="min-h-screen">
      ...
      <WaybillDialog
        open={showWaybillDialog}
        onClose={closeWaybillDialog}
        ...
        <DialogContent className="max-w-[90vw] lg:max-w-7xl max-h-[95vh] overflow-y-auto p-0">
          <ProfileCard 
            ...
            onDelete={async () => {
              ...
              toast.info('Профіль деактивовано');
