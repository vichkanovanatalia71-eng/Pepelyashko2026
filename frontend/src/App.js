...
<body>
  <div class="wrap">
    ...
    <div class="sec">
      ...
      <p class="num">7.1. Загальна вартість за Договором: <strong>{{total_amount_number}} грн ({{total_amount_in_words}})</strong> {{vat_note}}. Деталізація по позиціях – у Специфікації.</p>
...
<body>
  ...
  const saveContractTemplate = () => {
    ...
    localStorage.setItem('contractTemplateVersion', '5.1');
...
<body>
  ...
  const saveInvoiceTemplate = () => {
    ...
    localStorage.setItem('invoiceTemplateVersion', '8.0');
                  ...
                  <div className="border-2 border-gray-300 rounded-lg overflow-hidden bg-gray-100" style={{height: '500px'}}>
                    {contractPdfData.drive_file_id ? (
                      ...
                      <iframe
                        ...
                        allow="autoplay"
                      />
                    ) : contractPdfData.is_blob && contractPdfData.drive_view_link ? (
                  ...
                  <CardContent className="py-2">
                    <div className="space-y-3">
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  await axios.post(`${API}/invoices/${doc.number}/generate-pdf`);
                      ...
                      {relatedDocuments.waybills.map((doc, idx) => (
                        <div key={`wb-${idx}`} className="p-3 bg-orange-50 rounded border border-orange-200">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <input
                                type="checkbox"
                                checked={selectedRelatedDocs.some(d => d.type === 'waybill' && d.number === doc.number)}
                                onChange={(e) => {
                                  if (e.target.checked) {
                                    setSelectedRelatedDocs([...selectedRelatedDocs, { type: 'waybill', number: doc.number, doc }]);
                                  } else {
                                    setSelectedRelatedDocs(selectedRelatedDocs.filter(d => !(d.type === 'waybill' && d.number === doc.number)));
                                  }
                                }}
                                className="w-4 h-4 cursor-pointer"
                              />
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  // First, ensure PDF is generated (will use existing if available)
                                  await axios.post(`${API}/waybills/${doc.number}/generate-pdf`);
                      ...
                      {relatedDocuments.contracts.map((doc, idx) => (
                        <div key={`con-${idx}`} className="p-3 bg-blue-50 rounded border border-blue-200">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              <input
                                type="checkbox"
                                checked={selectedRelatedDocs.some(d => d.type === 'contract' && d.number === doc.number)}
                                onChange={(e) => {
                                  if (e.target.checked) {
                                    setSelectedRelatedDocs([...selectedRelatedDocs, { type: 'contract', number: doc.number, doc }]);
                                  } else {
                                    setSelectedRelatedDocs(selectedRelatedDocs.filter(d => !(d.type === 'contract' && d.number === doc.number)));
                                  }
                                }}
                                className="w-4 h-4 cursor-pointer"
                              />
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  if (!doc.drive_file_id) {
                                    const localPdfUrl = `${API}/contracts/pdf/${doc.number}`;
                                    try {
                                      const pdfResponse = await axios.get(localPdfUrl, { responseType: 'blob' });
                                      const blobUrl = URL.createObjectURL(pdfResponse.data);
                                      setContractPdfData({
