...
export default function TabBar({ tabs, activeTab, onChange }: TabBarProps) {
  return (
    <div
      className="flex items-center gap-2 mb-6 overflow-x-auto pb-1 -mb-1
                 scrollbar-none"
      role="tablist"
    >
      {tabs.map((tab) => (
        <button
          ...
          className={`px-4 py-2 rounded-xl text-sm font-medium border whitespace-nowrap
                      transition-all duration-200
                      active:scale-95
                      focus-visible:outline-2 focus-visible:outline-accent-400
                      ${activeTab === tab.id
                        ? "bg-accent-500/15 text-accent-400 border-accent-500/30 shadow-sm"
