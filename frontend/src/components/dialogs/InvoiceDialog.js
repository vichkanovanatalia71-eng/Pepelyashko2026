...
import { 
  Eye, Mail, Edit, Trash2, FileText, Calendar, Building2, 
  Package, X, Receipt
} from 'lucide-react';
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] lg:max-w-[1200px] p-0 bg-transparent border-none shadow-none overflow-visible max-h-[90vh]">
        ...
        <VisuallyHidden>
          <DialogTitle>Рахунок №{invoice.number}</DialogTitle>
          <DialogDescription>Деталі рахунку для {invoice.counterparty_name}</DialogDescription>
        </VisuallyHidden>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] text-gray-500 flex items-center gap-1">
                      ...
                      {formatDateUkrainian(invoice.date)}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-[500px]">
                  ...
                  <div className="grid grid-cols-4 gap-3 mb-4">
                    ...
                    <div className="p-2 bg-gray-50 rounded text-center">
                      ...
                      <p className="text-sm font-semibold">{formatDateUkrainian(invoice.date)}</p>
                    </div>
                    <div className="p-2 bg-gray-50 rounded text-center">
                      <p className="text-[10px] text-gray-500 uppercase">ЄДРПОУ</p>
                      <p className="text-sm font-mono">{invoice.counterparty_edrpou}</p>
                    </div>
                    <div className="p-2 bg-green-50 rounded text-center border border-green-200">
                      ...
                      <p className="text-sm font-bold text-green-600">{formatAmount(invoice.total_amount)} грн</p>
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="border border-gray-200 rounded-lg overflow-hidden">
                      <div className="bg-green-50 px-3 py-2 grid grid-cols-12 gap-1 text-[10px] font-semibold text-gray-500 uppercase">
                ...
                <div className="w-[320px] space-y-3">
                  ...
                  <div className="bg-green-50 rounded-lg p-3 border border-green-200">
                    ...
                    <div className="space-y-1 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-right max-w-[180px] truncate">{invoice.counterparty_name || '—'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500">ЄДРПОУ:</span>
                        <span className="font-medium">{invoice.counterparty_edrpou || '—'}</span>
                      </div>
                      {invoice.counterparty_email && (
                        <div className="flex justify-between">
                          <span className="text-gray-500">Email:</span>
                          <span className="truncate max-w-[150px]">{invoice.counterparty_email}</span>
                        </div>
                      )}
                      {invoice.counterparty_phone && (
                        <div className="flex justify-between">
                          <span className="text-gray-500">Тел:</span>
                          <span>{invoice.counterparty_phone}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-green-100 rounded-lg p-3 border border-green-300 text-center">
                    ...
                    <p className="text-xl font-bold text-green-700">{formatAmount(invoice.total_amount)} грн</p>
                  </div>

                  {/* Actions */}
                  <div className="space-y-2">
                    <Button onClick={onViewPDF} disabled={loading} className="w-full bg-green-500 hover:bg-green-600 text-white h-9">
                      <Eye className="w-4 h-4 mr-2" /> Переглянути PDF
                    </Button>
                    <Button variant="outline" onClick={onSendEmail} disabled={loading} className="w-full h-9 border-green-300 hover:bg-green-50">
                      <Mail className="w-4 h-4 mr-2 text-green-500" /> Надіслати Email
                    </Button>
                    <div className="flex gap-2">
                      {onEdit && (
                        <Button onClick={() => onEdit(invoice)} variant="outline" disabled={loading} className="flex-1 h-8 text-sm">
                  ...
                  <div className="border rounded-lg p-2 max-h-[150px] overflow-y-auto">
                    <CommentsSection entityType="invoice" entityId={invoice.number} compact />
                  </div>
                </div>
              </div>
            </div>
