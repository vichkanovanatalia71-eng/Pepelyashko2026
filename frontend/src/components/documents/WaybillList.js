...
const WaybillList = ({ 
  ...
  const filteredWaybills = filterByEdrpou(waybills);
  ...
  // Calculate total
  const totalAmount = sortedWaybills.reduce((sum, wb) => sum + (wb.total_amount || 0), 0);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>Список Накладних ({sortedWaybills.length})</span>
      ...
      <CardContent>
        ...
        {sortedWaybills.length === 0 ? (
          <p className="text-center py-8 text-gray-500">
            {searchValue ? 'Накладних за таким запитом не знайдено' : 'Немає накладних'}
          </p>
        ) : (
          <div className={getGridClass()}>
            {sortedWaybills.map((waybill) => (
              viewMode === 'list' ? (
                /* LIST VIEW */
                <div 
                  key={waybill._id} 
                ...
                >
                  <div className="flex items-center gap-2 min-w-[100px]">
                    <span className={`font-semibold ${theme.text}`}>№{waybill.number}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm truncate">{waybill.counterparty_name}</p>
                  </div>
                  <div className="text-right min-w-[90px]">
                    <p className={`font-bold text-sm ${theme.text}`}>{waybill.total_amount} грн</p>
                  </div>
                  <div className="text-xs text-gray-500 min-w-[80px] text-right">
                    {new Date(waybill.date).toLocaleDateString('uk-UA')}
                  </div>
                  <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                    <Button size="sm" variant="ghost" onClick={() => onViewWaybill(waybill)} className="h-7 px-2" title="Переглянути">
                    ...
                    <div className="flex justify-between items-start gap-2">
                      <div className="flex-1 min-w-0 overflow-hidden">
                        <div className="flex items-center justify-between gap-2">
                          <span className={`font-semibold ${theme.text} shrink-0`}>№{waybill.number}</span>
                          <span className="text-xs text-gray-500 shrink-0">
                            {new Date(waybill.date).toLocaleDateString('uk-UA')}
                          </span>
                        </div>
                        <p className="text-sm text-gray-600 truncate mt-1" title={waybill.counterparty_name}>{waybill.counterparty_name}</p>
                        <p className={`font-bold ${theme.text} mt-1`}>{waybill.total_amount} грн</p>
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" onClick={(e) => e.stopPropagation()}>
                        <Button size="sm" variant="ghost" onClick={() => onViewWaybill(waybill)} className="h-7 px-2">
                          <Eye className="w-4 h-4" />
                        </Button>
                      </div>
                    </div>
