...
const ActList = ({ 
  ...
  // Calculate total
  const totalAmount = sortedActs.reduce((sum, act) => sum + (act.total_amount || 0), 0);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>Список Актів ({sortedActs.length})</span>
        ...
        <div className="space-y-3 mb-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Input
                type="text"
                placeholder="Номер, контрагент, ЄДРПОУ або послуга"
                value={searchValue}
              ...
              viewMode === 'list' ? (
                ...
                >
                  <div className="flex items-center gap-2 min-w-[100px]">
                    <span className={`font-semibold ${theme.text}`}>№{act.number}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm truncate">{act.counterparty_name}</p>
                  </div>
                  <div className="text-right min-w-[90px]">
                    <p className={`font-bold text-sm ${theme.text}`}>{act.total_amount} грн</p>
                  </div>
                  <div className="text-xs text-gray-500 min-w-[80px] text-right">
                    {new Date(act.date).toLocaleDateString('uk-UA')}
                  </div>
                  <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                    <Button size="sm" variant="ghost" onClick={() => onViewAct(act)} className="h-7 px-2" title="Переглянути">
                      <Eye className="w-4 h-4" />
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => { onViewAct(act); setTimeout(() => onEditAct(act), 100); }} className="h-7 px-2" title="Редагувати">
                    ...
                    <div className="flex justify-between items-start gap-2">
                      <div className="flex-1 min-w-0 overflow-hidden">
                        <div className="flex items-center justify-between gap-2">
                          <span className={`font-semibold ${theme.text} shrink-0`}>№{act.number}</span>
                          <span className="text-xs text-gray-500 shrink-0">
                            {new Date(act.date).toLocaleDateString('uk-UA')}
                          </span>
                        </div>
                        <p className="text-sm text-gray-600 truncate mt-1" title={act.counterparty_name}>{act.counterparty_name}</p>
                        <p className={`font-bold ${theme.text} mt-1`}>{act.total_amount} грн</p>
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" onClick={(e) => e.stopPropagation()}>
                        <Button size="sm" variant="ghost" onClick={() => onViewAct(act)} className="h-7 px-2">
                          <Eye className="w-4 h-4" />
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => { onViewAct(act); setTimeout(() => onEditAct(act), 100); }} className="h-7 px-2">
                          <Edit2 className="w-4 h-4" />
                        </Button>
                      </div>
                    </div>
