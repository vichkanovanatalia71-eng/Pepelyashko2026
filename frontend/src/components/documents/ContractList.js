...
import { Search, ArrowUpDown, List, Grid2X2, Grid3X3, Eye } from 'lucide-react';
...
const ContractList = ({ 
  ...
  const filteredContracts = filterByEdrpou(contracts);
  ...
  // Calculate total
  const totalAmount = sortedContracts.reduce((sum, contract) => sum + (contract.amount || 0), 0);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>Список Договорів ({sortedContracts.length})</span>
      ...
      <CardContent>
        ...
        <div className="space-y-3 mb-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <Input
                type="text"
                placeholder="Номер, контрагент, ЄДРПОУ або предмет"
                value={searchValue}
        ...
        {sortedContracts.length === 0 ? (
          <p className="text-center py-8 text-gray-500">
            {searchValue ? 'Договорів за таким запитом не знайдено' : 'Немає договорів'}
          </p>
        ) : (
              ...
              viewMode === 'list' ? (
                ...
                >
                  <div className="flex items-center gap-2 min-w-[100px]">
                    <span className={`font-semibold ${theme.text}`}>№{contract.number}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm truncate">{contract.counterparty_name}</p>
                    <p className="text-xs text-gray-500 truncate">{contract.subject}</p>
                  </div>
                  <div className="text-right min-w-[90px]">
                    <p className={`font-bold text-sm ${theme.text}`}>{contract.amount} грн</p>
                  </div>
                  <div className="text-xs text-gray-500 min-w-[80px] text-right">
                    {contract.date ? new Date(contract.date).toLocaleDateString('uk-UA') : '-'}
                  </div>
                  <div className="flex gap-1" onClick={(e) => e.stopPropagation()}>
                    <Button size="sm" variant="ghost" onClick={() => onViewContract(contract)} className="h-7 px-2" title="Переглянути">
                    ...
                    <div className="flex justify-between items-start gap-2">
                      <div className="flex-1 min-w-0 overflow-hidden">
                        <div className="flex items-center justify-between gap-2">
                          <span className={`font-semibold ${theme.text} shrink-0`}>№{contract.number}</span>
                          <span className="text-xs text-gray-500 shrink-0">
                            {contract.date ? new Date(contract.date).toLocaleDateString('uk-UA') : '-'}
                          </span>
                        </div>
                        <p className="text-sm text-gray-600 truncate mt-1" title={contract.counterparty_name}>{contract.counterparty_name}</p>
                        <p className="text-xs text-gray-500 truncate" title={contract.subject}>{contract.subject}</p>
                        <p className={`font-bold ${theme.text} mt-1`}>{contract.amount} грн</p>
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" onClick={(e) => e.stopPropagation()}>
                        <Button size="sm" variant="ghost" onClick={() => onViewContract(contract)} className="h-7 px-2">
                          <Eye className="w-4 h-4" />
                        </Button>
                      </div>
                    </div>
